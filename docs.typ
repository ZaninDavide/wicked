...
This small Typst package handles the typesetting of Wick contractions $wick(pos: #top, phi.alt)^dagger wick(pos: #top, phi.alt)$. We shall not fall into the same typographical limitations that stopped the italian physicist Gian Carlo Wick from using this notation in his papers.    
...
#set box(width: 20pt, height: 20pt, radius: 2pt)
```
The hole package functionality is contained within the `wick` function. By calling `wick(..)` on some content you place a _contraction point_. Every second contraction point you place, gets contracted with the previous one.
...
The `id` is not required to be an integer; for example `string`s are another reasonable choice. That being said, integer `id`s have a special behaviour. Did you notice how the outer contraction in the last example automatically moved further away? When `id` is of type `int`, the distance of the contraction line, which is otherwise specified by the parameter `dy`, is instead computed as `dy * (1 + id / 2.0)`.
...
```
$
#wick(box(fill: red), dx: 5pt)#h(3pt)
